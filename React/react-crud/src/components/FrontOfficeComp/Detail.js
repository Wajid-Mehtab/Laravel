...
const Api = {
  ...
  createComplaint: (payload) => axios.post(`${API_BASE}/complaints`, payload),
};
...
function getProceedMode(df) {
  const hasDept = df?.SindhGovDept && df.SindhGovDept !== "--Select--" && df.SindhGovDept.trim() !== "";
  const hasQuery = (df?.Query || "").trim().length > 0;
  const hasSolution = (df?.Solution || "").trim().length > 0;
  return hasDept && hasQuery && hasSolution ? "Complete" : "Initiated";
}
...
export default function DetailTab({
  ...
  function buildCreatePayload(detailForm, { oldTagOptions, mflList, wrlmpList }, { phoneNo, callerName }) {
    // normalize ids
    const oldIds   = Array.isArray(detailForm.OldTagIds)   ? detailForm.OldTagIds   : [];
    const mflIds   = Array.isArray(detailForm.MFLIds)      ? detailForm.MFLIds      : [];
    const wrlmpIds = Array.isArray(detailForm.WRLMPIds)    ? detailForm.WRLMPIds    : [];

    // convert IDs → NAMES (comma-separated) like WinForms did (Items[i].Text)
    const MainTags  = namesFromIds(oldIds,   oldTagOptions).join(", ");
    const TagMFL    = namesFromIds(mflIds,   mflList).join(", ");
    const TagWRLMP  = namesFromIds(wrlmpIds, wrlmpList).join(", ");

    // flags → strings (your WinForms assigned text on check/uncheck)
    const AdditionalInfo1  = detailForm.ADRQuery           ? "ADR Query"              : " ";
    const COVID            = detailForm.COVID              ? "COVID-19"               : " ";
    const AdditionalInfo2  = detailForm.WomenRelated       ? "Women Related"          : " ";
    const AgeAboveFifty    = detailForm.AgeAboveFifty      ? "Age > 50"               : " ";
    const Domesticviolence = detailForm.Domesticviolence   ? "Gender Base Violence"   : " ";
    const WomenProperty    = detailForm.WomenProperty      ? "Women Property"         : " ";

    // Category IDs: WinForms set to 0 if “--Select--”
    const categoryId     = detailForm.CategoryID       ?? 0;
    const subCategoryId  = detailForm.SubCategoryID    ?? 0;
    const childCatId     = detailForm.ChildCategoryID  ?? 0;

    // Backend columns used by your C# insert; we send the same *keys it expects*
    // If your Laravel POST expects different keys, keep the *names* you used for PUT (update)
    return {
      // Essentials
      ContactPerson:  callerName || detailForm.ContactPerson || "",
      ContactNumber:  phoneNo    || detailForm.ContactNumber || "",

      CategoryName:    detailForm.MainCat        || "",
      SubCategoryName: detailForm.Category       || "",
      ComplaintSource: detailForm.ComplaintSource || "",
      Priority:        detailForm.Priority       || "Normal",
      ...
      // child info
      AdditionalInfo4: childCatId,                 // childCatID
      AdditionalInfo5: detailForm.SubCatChild||"", // childCatName (text)
      ...
      // Job status on create (WinForms set “Initiated”)
      JobStatus: "Initiated",

      // CSV tags (names, not IDs)
      MainTags,
      TagMFL,
      TagWRLMP,

      // Optional: who created (match your app’s auth/user, if available)
      CreatedBy: detailForm.Created || "",
    };
  }
  ...
  const handleProceedClick = useCallback(async (mode /* "Initiated" | "Complete" */) => {
    // 1) Generic source validation (same as C# top)
    if (!detailForm.ComplaintSource || detailForm.ComplaintSource === "--Select--") {
      setErrors(["Please select Source."]);
      return;
    }

    // 2) Mode-specific validations
    if (mode === "Complete") {
      if (!detailForm.SindhGovDept || detailForm.SindhGovDept === "--Select--") {
        setErrors(["Sindh Government department can't be empty!"]);
        return;
      }
      // your WinForms ValidateQueryData(); replicate key checks you need:
      const missing = [];
      if (!detailForm.Query || detailForm.Query.trim() === "") missing.push("Query");
      if (!detailForm.Solution || detailForm.Solution.trim() === "") missing.push("Solution");
      if (missing.length) {
        setErrors([`Please fill: ${missing.join(", ")}`]);
        return;
      }
    }
    ...
    if (mode === "Initiated") {
      // phone & name required
      const phone = (detailForm.ContactNumber || "").trim();
      const name  = (detailForm.ContactPerson || "").trim();
      if (!phone) {
        setErrors(["Phone No. can't be empty"]);
        return;
      }
      if (!name) {
        setErrors(["Please enter caller name."]);
        return;
      }

      // Normalize IDs like WinForms (set to 0 when “--Select--”)
      setDetailForm((prev) => ({
        ...prev,
        CategoryID:      prev.Category ? (prev.CategoryID ?? 0)      : 0,
        SubCategoryID:   prev.Category ? (prev.SubCategoryID ?? 0)   : 0,
        ChildCategoryID: prev.SubCatChild ? (prev.ChildCategoryID ?? 0) : 0,
      }));
    }

    // 3) Build payload (names CSV, same as C# did via Items[i].Text)
    const payload = buildCreatePayload(
        { ...detailForm },
        { oldTagOptions, mflList, wrlmpList },
        { phoneNo, callerName }
    );

    // 4) Fire POST /complaints
    try {
      setSaving(true);
      console.log("🟢 Creating complaint with payload:", payload);
      const res = await Api.createComplaint(payload);

      // Try multiple shapes { id }, { LastQID }, { data: { id } }
      const newId =
          res?.data?.id ??
          res?.data?.LastQID ??
          res?.data?.ComplaintID ??
          res?.data?.data?.id ??
          res?.data?.data?.ComplaintID;

      if (!newId) {
        console.warn("Create returned no id; raw:", res?.data);
        alert("Created, but no id returned.");
        return;
      }

      // 5) Update state like WinForms did after insert
      setDetailForm((prev) => ({
        ...prev,
        QueryID: String(newId),
        JobStatus: "Initiated",
        CreatedOn: prev.CreatedOn || new Date().toISOString().slice(0, 10),
        Created: prev.Created || "", // fill if you track user
      }));

      // Cache reset + one-time selections fetch after create
      const k = String(newId);
      selectionsCache.delete(k);
      loadedSelectionsRef.current = false;
      selectionsFetchStateRef.current = "idle";
      await fetchSelectionsOnce(true);

      // Flip UI: your button bar already uses QueryID to switch modes
      alert(`Query created. ID: ${newId}`);
    } catch (err) {
  ...
  function handleNewQuery() {
    try {
      // clear errors & local loaders
      setErrors([]);
      setSaving(false);

      // uncheck all tag buckets (and clear arrays on the form)
      uncheckAll();
      ...
      // clear selection-cache guards (fresh start)
      loadedSelectionsRef.current = false;
      selectionsFetchStateRef.current = "idle";

      // reset the form (mirrors your CleanQueryDetailsSection + defaults)
      setDetailForm({
        QueryID: "",                 // no ID yet
        JobStatus: "Initiated",      // default like WinForms
        Created: "",                 // Launched By
        Modified: "",                // Accepted By
        CreatedOn: "",               // date
        ModifiedOn: "",              // date
        ...
        // flags (match WinForms text semantics via your builders)
        ADRQuery: false,
        COVID: false,
        WomenRelated: false,
        AgeAboveFifty: false,
        ...
        // priority & source defaults
        Priority: "Normal",
        ComplaintSource: "",         // WinForms sets SelectedIndex=0 ("--Select--")
        SindhGovDept: sindhGovDepartments[0] ?? "--Select--",
        ...
        // legacy/compat fields (kept blank/zero)
        OldTagId: "",
        MFL: "",
        WRLMP: "",
        AdditionalInfo3: "0",
      ...
      // button visibility like WinForms: hide Save, show Proceed
      setIsNewQuery(true);

      // focus main category, like cbCatDetails.Focus();
      setTimeout(() => catSelectRef.current?.focus(), 0);
    } catch (err) {
        ...
        <div className="card p-3 shadow-sm rounded-3">
          ...
          <div className="row g-1">
            ...
            <div className="col-12 text-end mt-3">
              ...
              {/* TODO: wire these when your endpoints are ready */}
              <button
                  className="btn btn-success btn-sm me-2"
                  onClick={() => {
                    // send SMS flow (e.g., POST /complaints/:id/sms)
                    // for now just surface a friendly nudge
              ...
              <button className="btn btn-outline-primary btn-sm me-2" onClick={handleNewQuery}>
                New Query
              </button>
              ...
              <button
                  className="btn btn-success btn-sm me-2"
                  onClick={() => {
                    // view notes UI (modal / navigate)
                    alert("View Notes coming soon.");
                  }}
              >
                View Notes
              </button>
              {/* one smart proceed button */}
              <button
                  className="btn btn-primary btn-sm me-2"
                  onClick={() => handleProceedClick(getProceedMode(detailForm))}
                  disabled={saving}
                  title={
                    getProceedMode(detailForm) === "Complete"
                        ? "All key fields present: will create as Complete"
                        : "Missing Complete fields: will create as Initiated"
                  }
              >
                {saving ? "Working..." : `Proceed (${getProceedMode(detailForm)})`}
              </button>

              {/* show Save only when editing an existing complaint (has QueryID) */}
              {Boolean(detailForm?.QueryID && !isNewQuery) && (
