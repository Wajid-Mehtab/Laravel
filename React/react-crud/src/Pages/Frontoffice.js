...
import { SAVE_CONTACT_URL,Update_CONTACT_URL } from "../config/api.js";
...
export default function Frontoffice() {
  ...
  // 1) Get user FIRST
  const { user, setUser, clearUser } = useSessionUser();
  ...
  const lsUser = (() => {
   ...
   const createdByForReminders =
         (lsUser?.UserName && String(lsUser.UserName).trim()) ||
         (username && String(username).trim()) ||        // SessionContext username
         (lsUser?.FullName && String(lsUser.FullName).trim()) ||
         '';
  // EXAMPLE: call this after your login succeeds
  async function mockLogin() {
    // you’ll probably get these from your login API
    setUser({ userId: 42, username: 'Rabia Binte-e-Rashid', roleName: 'FOE' });
  }
  ...
  const historyQid =
      detailForm?.QueryID ||
      form?.complaintid ||   // you set this in handleRowClick
      form?.QueryID ||
      form?.Queryid ||
      "";
// Example usage in saving:
     ...
     // console.log("fetchContactDetails", text);
      ...
      if (result.status === "known" && result.contacts.length > 0) {
        ...
        setForm((prev) => ({
          ...
          AdditionalInfo2: contact.District || "",
          AdditionalInfo3: contact.Province || "",
          AdditionalInfo4: contact.Country || "",
          AdditionalInfo5: contact.HeardFrom || "",
          AdditionalInfo6: contact.Religion || "",
          AdditionalInfo8: contact.MaritalStatus || "",
  ...
  const fetchComplaintDetails = async (complaintID) => {
    try {
      const res = await fetch(`http://localhost:8000/api/DetailEditQueryForGrid/${complaintID}`);
      const result = await res.json();
      if (Array.isArray(result) && result.length > 0) {
        const r = result[0];

        // Map API -> UI field names so inputs show values
        setDetailForm({
          ...r,
          QueryID: r.QueryID,                 // same
          MainCat: r.Category || "",          // CategoryName AS Category
          Category: r.SubCategory || "",      // SubCategoryName AS SubCategory
          SubCategory: r.SubCatChild || "",   // AdditionalInfo5 AS SubCatChild
          ...
          // Booleans derived from the strings you store in DB
          ADRQuery: (r.AdditionalInfo1 || "").trim() === "ADR Query",
          COVID: (r.COVID || "").trim() === "COVID-19",
          AgeAboveFifty: (r.AgeAboveFifty || "").trim() === "Age > 50",
          Domesticviolence: (r.Domesticviolence || "").trim() === "Gender Base Violence",
  ...
  const activeQid =
      ...
      form?.complaintid ||
      form?.QueryID ||
      form?.Queryid ||   // note: you set this in handleRowClick
      "";
  ...
  const handleRowClick = (rowData) => {
    ...
    setForm((prev) => ({
      ...prev,
      Queryid: rowData.QID || "", // <- required for update
      CallerID: rowData.CallersID || "", // <- if needed for display
      PhoneNo: rowData.ContactNumber || "",
      Name: rowData.ContactPerson || "",
      CNIC: rowData.CNIC || "",
      City: rowData.City || "",
      Address: rowData.Address || "",
      complaintid: rowData.QID || "",
      CreatedBy: userid,
    }));

    setSelectedTab("callerInfo");

    // fetchContactDetails(rowData.ContactNumber, rowData.QueryID || -1);
    //  fetchComplaintDetails(rowData.CallerID);
    fetchContactDetails(rowData.ContactNumber, rowData.queryId || -1);

    // If you still need to fetch complaint data by QID, you can keep this
    // fetchComplaintDetails(rowData.QID);
    fetchComplaintDetails(rowData.QID);
  };
  ...
  const handleUpdateContact = async () => {
    ...
    console.error("form.id is missing", form.id);
    return alert("No contact selected to update.");
    // try {
    //   console.log("Updating contact with ID:", form.id);
    //   const res = await fetch(Update_CONTACT_URL(form.id), {
    //     method: "PUT",
    //     headers: { "Content-Type": "application/json" },
  ...
  const handleClear = () => {
    if (window.confirm("Clear the form?")) {
      setForm({ ...initialForm, CreatedBy: userid }); // ✅ keep CreatedBy
      setDetailForm({});                     // <-- clear detail tab too
      setListData([]);
    }
  };
  ...
  const fetchCategories = async () => {
    try {
      const res = await fetch("http://localhost:8000/api/complaint-categories");
      const json = await res.json();
      if (json?.success && Array.isArray(json.data)) {
        setCategories(json.data); // [{id, name}, ...]
      } else {
        setCategories([]);
        console.error("Bad categories response:", json);
      }
    } catch (e) {
  ...
  const renderContent = () => {
    switch (selectedTab) {
      ...
      case "notes":
        return <NotesTab queryId={activeQid} currentUserId={userid}  />;
      case "viewhistory":
        return <ViewHistoryTab queryId={historyQid} />;
     // case "reminders": return <RemindersTab createdBy={createdByForReminders} />;
     case "reminders": return <RemindersTab createdBy={username} />;
        ...
        <div className="row">
          ...
          <div className="col-md-10 p-3">
            <div className="row bg-white py-2 border-bottom mb-3">
              ...
              <div className="col-md-4 text-center"><strong>Query ID:</strong> {form.complaintid}</div>
              <div className="col-md-4 text-end"><strong>Caller ID:</strong> {form.CallerID}</div>
            </div>
            {renderContent()}
          </div>
        </div>
